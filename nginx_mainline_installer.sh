...
EOF
        ...
        fedora)
            ...
            cat > "$REPO_FILE" <<EOF
[nginx-mainline]
name=NGINX Mainline for Fedora $version_id
baseurl=http://nginx.org/packages/mainline/fedora/$version_id/\$basearch/
gpgcheck=1
enabled=1
gpgkey=${NGINX_GPG_KEY_URL}
EOF
...
EOF
        ...
        debian)
            ...
            add_debian_key
            echo "deb ${KEY_OPT} http://nginx.org/packages/mainline/debian $codename nginx" \
                > /etc/apt/sources.list.d/nginx.list
            echo "deb-src ${KEY_OPT} http://nginx.org/packages/mainline/debian $codename nginx" \
                >> /etc/apt/sources.list.d/nginx.list
            ;;
        ...
        ubuntu)
            codename=$(lsb_release -cs)
            add_debian_key
            echo "deb ${KEY_OPT} http://nginx.org/packages/mainline/ubuntu $codename nginx" \
                > /etc/apt/sources.list.d/nginx.list
            echo "deb-src ${KEY_OPT} http://nginx.org/packages/mainline/ubuntu $codename nginx" \
                >> /etc/apt/sources.list.d/nginx.list
            ;;
...
case "$distro" in
    debian|ubuntu)
        apt-get install -y nginx \
          || print_error "apt install failed."
        ;;
...
###############################################
# Verify installed version (1.27.x)
###############################################
